...
interface AddVehicleFormProps {
  onAdd: (vehicleNo: string, driverName: string, driverPhone: string, isActive: boolean) => Promise<{ success: boolean; error?: string }>;
}
...
export function AddVehicleForm({ onAdd }: AddVehicleFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await onAdd(vehicleNo.trim(), driverName.trim(), driverPhone.trim(), isActive);
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-5 shadow-sm flex flex-col md:flex-row gap-4 items-end">
      ...
      <div className="flex-1 w-full">
        <label className="text-xs font-semibold text-foreground/60 mb-1.5 block uppercase tracking-wider">Driver Phone</label>
        <input 
          type="text" 
