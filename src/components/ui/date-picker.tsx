...
export function DatePicker({ value, onChange, showTicketCounts = false }: DatePickerProps) {
  ...
  useEffect(() => {
    ...
    if (open && showTicketCounts) {
      fetchCounts();
    }
  }, [currentMonth, open, showTicketCounts]);
  ...
  const displayDate = value ? format(safeDateParse(value), "MMM d, yyyy") : "Select Date";

  return (
    <Popover open={open} onOpenChange={(isOpen) => {
      setOpen(isOpen);
      if (isOpen) setCurrentMonth(safeDateParse(value));
    }}>
      <PopoverTrigger className="flex items-center gap-2 px-3 py-2 text-sm bg-background border border-border shadow-sm outline-none cursor-pointer hover:bg-muted rounded-lg transition-colors w-full justify-start text-left">
        <CalendarIcon className="w-4 h-4 text-muted-foreground" />
        <span className="font-medium text-foreground">{displayDate}</span>
      </PopoverTrigger>
      <PopoverContent className="w-[260px] p-0 shadow-xl flex flex-col mt-1 bg-card" align="start">
        ...
        <div className="grid grid-cols-7 gap-1 px-3 pb-3">
          ...
          {days.map(d => {
            ...
            return (
              ...
              >
                <span>{d}</span>
                {showTicketCounts && ticketCounts[dateStr] > 0 && (
